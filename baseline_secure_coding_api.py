...
def get_profile(student_id):
    if flask.session.get("role") != "student" or flask.session.get("student_id") != int(student_id):
...
@login_required
def update_grade():
    if flask.session.get("role") != "professor":
        return {"error": "forbidden"}, 403
...
def download_file():
    ...
    filename = pathlib.Path(raw_path).name
    target = (MATERIALS_DIR / filename).resolve()
    
    if not target.starts_with(MATERIALS_DIR.resolve()) or not target.exists():
